...
- 已准备的批次在重试期间保持内容/ID 不变，条件写入及远端 SHA 校验避免重复覆盖。归档失败不出队；离线恢复并核实存储时间后清理超期队列记录，输出 `expired_unarchived_events`，不能假称它们成功归档。
...
时间异常处理：每轮采集先校验存储端 UTC，再以经过校验的本地时间和单调时钟推进接收时间；本地过期判断使用存储端时间。已知时钟偏差超过5分钟（包括S3拒绝错时签名）会暂停本轮读入，不推进源游标。存储不可用时仍可写入有容量上限的本地缓冲，但暂不清理或封装新批次，恢复后再按可信时间处理；不会无限扩充缓冲。

离线启动且尚未校时时，新记录明确标为 `receipt_time_status=unverified`，并在 `unverified_observed_at` 保留原读数。首次成功校时后，仅对这些未封包记录采用首次可信接收时间开始30天保留期，标为 `recovered`；原始读数、source_time和event_id不变。这是恢复接收时间，不冒充实际操作时刻。原时间字段预留固定宽度，因此满缓冲也能恢复。

已校时或已封包的未来时间戳记录不会自动改写、删除，也不会挡住正常批次；输出 `future_records_pending_clock_review`，时间追上后可继续。历史异常时间数据可能需要运维核查。一个来源超过256文件只计为该来源不可用，其他来源继续采集。Uvicorn访问日志的IPv6按真实socket格式分离末尾端口，不把端口拼成访问IP。
